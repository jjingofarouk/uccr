...
const fetchUserPhotoURL = async (uid) => {
  try {
    ...
    return profileSnap.exists() ? profileSnap.data().photoURL || '/images/doctor-avatar.jpeg' : '/images/doctor-avatar.jpeg';
  } catch (error) {
...
export const getCases = async (uid = null) => {
  try {
    ...
    const cases = await Promise.all(querySnapshot.docs.map(async (doc) => {
      const data = doc.data();
      const photoURL = await fetchUserPhotoURL(data.userId);
      return {
        id: doc.id,
        userId: data.userId || '',
...
export const getProfile = async (uid) => {
  try {
    ...
    return {
      ...
      professionalAffiliations: Array.isArray(profileData.professionalAffiliations) ? profileData.professionalAffiliations : [],
...
export const getTopContributors = async (limitCount = 3) => {
  try {
    ...
    for (const profileDoc of profilesSnapshot.docs) {
      ...
      const casesQuery = query(collection(db, 'cases'), where('userId', '==', profileDoc.id));
      const casesSnapshot = await getDocs(casesQuery);
      const caseCount = casesSnapshot.size;
      let totalAwards = 0;
      casesSnapshot.forEach(doc => {
        totalAwards += Number(doc.data().awards) || 0;
      });
      const awards = [];
      if (totalAwards >= 5) awards.push('Gold');
      else if (totalAwards >= 3) awards.push('Silver');
      else if (totalAwards >= 1) awards.push('Bronze');
      if (caseCount > 0) {
        contributors.push({
          uid: profileDoc.id,
    ...
    return contributors.sort((a, b) => b.caseCount - a.caseCount).slice(0, limitCount);
  } catch (error) {
    return [];
  }
};
    ...
    const casePromises = casesSnapshot.docs.map(async (doc) => {
      ...
      if (matches) {
        return {
          ...
          specialty: Array.isArray(data.specialty) ? data.specialty : [data.specialty] || [],
          userId: data.userId,
          createdAt: data.createdAt?.toDate() || new Date(),
        };
      }
